...
```mermaid
graph TD
    Start["مشروع مقترح للإدراج (Candidate Repo)"] --> Q1{"هل المستودع عام ومفتوح المصدر بكود فعلي؟"}
    ...
    Q3 -- نعم --> Reject
    Q3 -- لا --> Q4{"هل هو أداة عالمية عامة متعددة اللغات (مثل EasyOCR)؟"}
    Q4 -- نعم --> Reject
    Q4 -- لا --> Q5{"هل الغرض الأساسي حوسبة أو خدمة أو تعليم العربية؟"}
    Q5 -- لا --> Reject
    Q5 -- نعم --> Q6{"هل يندرج تحت أحد التصنيفات الـ 7؟"}
    Q6 -- نعم --> Accept["✅ قبول المشروع وإدراجه في الكتالوج"]
    Q6 -- لا --> Reject
```
...
sequenceDiagram
    autonumber
    actor Contributor as المساهم (Contributor)
    participant GH as GitHub Issue
    participant Actions as GitHub Actions Bot
    actor Maintainer as مدير المشروع (Maintainer)
    
    Contributor->>GH: تعبئة نموذج اقتراح مشروع جديد
    GH->>Actions: إطلاق الأكشن process-submission.yml
    Actions->>Actions: استخراج البيانات والتحقق عبر GitHub API
    Actions->>Actions: جلب النجوم، الرخصة، والتصنيفات وتحديث projects.json
    Actions->>Actions: تشغيل فحص المطابقة pnpm validate
    Actions->>GH: فتح Pull Request آلي + وضع تعليق برابطه في التذكرة
    Maintainer->>Actions: مراجعة طلب السحب واعتماده بضغطة واحدة (Merge)
    Actions->>Actions: نشر التحديث مباشرة على الموقع الحي
```
